...
impl<R: BufRead> StreamingDDEXParser<R> {
    ...
    fn handle_end_element(&mut self, element: &quick_xml::events::BytesEnd) -> Result<Option<ParsedElement>, ParseError> {
        ...
        let result = match &mut self.context.state {
            ParserState::InHeader { header, depth } => {
                self.handle_header_end_element(name, &text_content, header, *depth)?
            }
            ParserState::InRelease { release, depth } => {
                self.handle_release_end_element(name, &text_content, release, *depth)?
            }
            ParserState::InResource { resource, depth } => {
                self.handle_resource_end_element(name, &text_content, resource, *depth)?
            }
            ParserState::InParty { party, depth } => {
                self.handle_party_end_element(name, &text_content, party, *depth)?
            }
            ParserState::InDeal { deal, depth } => {
                self.handle_deal_end_element(name, &text_content, deal, *depth)?
            }
        ...
        match name {
            ...
            "MessageHeader" if self.context.current_depth == depth => {
                ...
                let element = HeaderBuilder::new()
                    ...
                    .build()
                    .map_err(|e| ParseError::XmlError {
                        message: format!("Failed to build header: {}", e),
                        location: self.get_current_location(),
                    })?;
